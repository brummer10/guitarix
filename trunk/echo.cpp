...
class echodsp : public dsp {
  ...
  public:
	...
	virtual void buildUserInterfaceech(UI* interface) {
		...
		interface->addVerticalSlider("time", &fslider0, 0.0f, 0.0f, 2000.0f, 1.0f);
		interface->closeBox();
	}
...
{		
 ...
 public:
	...
	virtual void addVerticalSlider(const char* label, float* zone, float initech, float min, float max, float step) { 
		addPortDescrechech(ICONTROL, label, LADSPA_HINT_BOUNDED_BELOW | LADSPA_HINT_BOUNDED_ABOVE, min, max); 
	}		
		
	virtual void addHorizontalSlider(const char* label, float* zone, float initech, float min, float max, float step) { 
		addPortDescrechech(ICONTROL, label, LADSPA_HINT_BOUNDED_BELOW | LADSPA_HINT_BOUNDED_ABOVE, min, max); 
	}
