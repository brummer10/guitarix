...
Widget::Widget(Glib::ustring plug_name)
{
  ...
  m_paintbox.set_name(plug_name);
    m_paintbox.set_size_request(360,260);
  ...
  make_controller_box(&m_vbox, &m_bigknob, "gain", 0, 20, 0.1, gain, plug_name);
  m_bigknob.signal_value_changed().connect(sigc::mem_fun(*this,
      &Widget::on_knob_value_changed));
  
  make_controller_box(&m_vbox1, &m_bigknob1, "tone", -6, 6, 0.1, tone, plug_name);
  m_bigknob1.signal_value_changed().connect(sigc::mem_fun(*this,
      &Widget::on_knob1_value_changed));

  make_controller_box(&m_vbox2, &m_bigknob2, "drive", 0, 20, 0.1, drive, plug_name);
...
void Widget::on_knob_value_changed()
{
  ...
  write_function(controller, (EffectPortIndex)GAIN,
...
void Widget::on_knob1_value_changed()
{
  ...
  write_function(controller, (EffectPortIndex)TONE,
...
void Widget::on_knob2_value_changed()
{
  ...
  write_function(controller, (EffectPortIndex)DRIVE,
