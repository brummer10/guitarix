...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win->label = "GxRoom Simulator";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], ROOMSIZE,"Rommsize", ui,40, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.0, 0.0, 1.0, 3.0, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], PREDELAYMS,"Pre Delay", ui,160, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,20.0, 20.0, 1.0, 200.0, 1.0, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], RT,"FB", ui,280, 25,100, 125);
    set_adjustment(ui->widget[2]->adj,0.3, 0.3, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], DRYWET,"Dry/Wet", ui,400, 25, 100, 125);
    set_adjustment(ui->widget[3]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[4] = add_my_toggle_button(ui->widget[4], EFFECT,"Power", ui,630, 70, 70, 40);
