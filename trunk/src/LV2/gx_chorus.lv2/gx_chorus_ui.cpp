...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri) {
    (*w) = 640; //set initial width of main window
    (*h) = 180; //set initial height of main window
}

const char* plugin_set_name() {
    return "GxChorus-Stereo"; //set plugin name to display on UI
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri) {
    ...
    ui->widget[0] = add_my_knob(ui->widget[0], LEVEL,"Level", ui,130, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], DELAY,"Delay", ui,255, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,0.02, 0.02, 0.0, 0.2, 0.001, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], DEPTH,"Depth", ui,380, 25, 100, 125);
    set_adjustment(ui->widget[2]->adj,0.02, 0.02, 0.0, 1.0, 0.005, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], FREQ,"Freq", ui,500, 25, 100, 125);
    set_adjustment(ui->widget[3]->adj,3.0, 3.0, 0.1, 10.0, 0.05, CL_CONTINUOS);

    ui->widget[4] = add_on_off_button(ui->win, "Power", 50, 50, 40, 80);
