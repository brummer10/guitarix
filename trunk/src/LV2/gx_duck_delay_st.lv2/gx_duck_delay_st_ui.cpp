...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri) {
    (*w) = 910; //set initial widht of main window
    (*h) = 180; //set initial heigth of main window
}

const char* plugin_set_name() {
    return "GxDuckDelay-Stereo"; //set plugin name to display on UI
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri) {
    ...
    ui->widget[0] = add_my_knob(ui->widget[0], TIME,"Time", ui,110, 35, 80, 105);
    set_adjustment(ui->widget[0]->adj,500.0, 500.0, 1.0, 2000.0, 1.0, CL_CONTINUOS);
    adj_set_scale(ui->widget[0]->adj, 10.0);

    ui->widget[1] = add_my_knob(ui->widget[1], FEEDBACK,"Feedback", ui,210, 35, 80, 105);
    set_adjustment(ui->widget[1]->adj,0.0, 0.0, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], PINGPONG,"PinPong", ui,310, 35, 80, 105);
    set_adjustment(ui->widget[2]->adj,0.0, 0.0, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], COLORATION,"Coloration", ui,410, 35, 80, 105);
    set_adjustment(ui->widget[3]->adj,0.0, 0.0, -1.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[4] = add_my_knob(ui->widget[4], ATTACK,"Attack", ui,510, 35, 80, 105);
    set_adjustment(ui->widget[4]->adj,0.1, 0.1, 0.05, 0.5, 0.001, CL_CONTINUOS);
    adj_set_scale(ui->widget[4]->adj, 2.0);

    ui->widget[5] = add_my_knob(ui->widget[5], RELEASE,"Release", ui,610, 35, 80, 105);
    set_adjustment(ui->widget[5]->adj,0.1, 0.1, 0.05, 2.0, 0.01, CL_CONTINUOS);
    adj_set_scale(ui->widget[5]->adj, 2.0);

    ui->widget[6] = add_my_knob(ui->widget[6], AMOUNT,"Amount", ui,710, 35, 80, 105);
    set_adjustment(ui->widget[6]->adj,0.5, 0.5, 0.0, 56.0, 0.1, CL_CONTINUOS);
    adj_set_scale(ui->widget[6]->adj, 5.0);

    ui->widget[7] = add_my_knob(ui->widget[7], EFFECT,"Effect", ui,810, 35, 80, 105);
    set_adjustment(ui->widget[7]->adj,0.0, 0.0, -16.0, 4.0, 0.1, CL_CONTINUOS);
    adj_set_scale(ui->widget[7]->adj, 2.0);

    ui->widget[8] = add_on_off_button(ui->win, "Power", 50, 50, 40, 80);
    ui->widget[8]->scale.gravity = ASPECT;
    ui->widget[8]->data = BYPASS;
    ui->widget[8]->parent_struct = ui;
    ui->widget[8]->func.value_changed_callback = value_changed;
}
