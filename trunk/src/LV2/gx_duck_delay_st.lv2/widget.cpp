...
logo("Stereo Duck Delay")
{
  ...
  make_controller_box(&m_vbox3, "FEEDBACK", 0.0, 1.0, 0.001, FEEDBACK);
  make_controller_box(&m_vbox4, "PINGPONG", 0.0, 1.0, 0.001, PINGPONG);
  make_controller_box(&m_vbox5, "COLORATION", -1.0, 1.0, 0.1, COLORATION);
  make_controller_box(&m_vbox6, "ATTACK", 0.05, 0.5, 0.01, ATTACK);
  make_controller_box(&m_vbox7, "RELEASE", 0.05, 2.0, 0.001, RELEASE);
  make_controller_box(&m_vbox8, "AMOUNT", 0, 56.0, 0.1, AMOUNT);
  make_controller_box(&m_vbox9, "EFFECT", -16.0, 4.0, 0.01, EFFECT);
