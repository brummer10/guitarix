...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 225-tw, 170 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,350,10);
    cairo_paint (w->crb);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 450, 180);
    ui->win->parent_struct = ui;
    ui->win->label = "GxFuzzMaster";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], VOLUME,"Volume", ui,40, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.1, 0.1, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], TONE,"Tone", ui,175, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], WET_DRY,"Dry/Wet", ui,310, 25, 100, 125);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 450, 180);
    }
