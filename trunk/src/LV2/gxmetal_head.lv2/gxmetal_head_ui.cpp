...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        .bg =       { 0.10, 0.19, 0.32, 1.00},
        .base =     { 0.10, 0.19, 0.32, 1.00},
...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 270-tw, 170 );
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 540, 180);
    ui->win->parent_struct = ui;
    ui->win->label = "GxMetalHead";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], PREGAIN,"Pre Gain", ui,40, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.0, 0.0, -20.0, 20.0, 0.1, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], DRIVE,"Drive", ui,160, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,0.32, 0.32, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], TONE,"Tone", ui,280, 25, 100, 125);
    set_adjustment(ui->widget[2]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], GAIN1,"Level", ui,400, 25, 100, 125);
    set_adjustment(ui->widget[3]->adj,0.0, 0.0, -20.0, 20.0, 0.1, CL_CONTINUOS);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 540, 180);
    }
