...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        .bg =       { 0.11, 0.11, 0.05, 1.00},
        .base =     { 0.11, 0.11, 0.05, 1.00},
    ...
    main->color_scheme->prelight = (Colors) {
        ...
        .base =     { 0.11, 0.11, 0.21, 1.00},
        .text =     { 0.11, 0.11, 0.21, 1.00},
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win->label = "Gxgcb_95";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], HOTPOTZ,"Wah", ui,40, 25, 80, 105);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], VOLUME,"Volume", ui,140, 25, 80, 105);
    set_adjustment(ui->widget[1]->adj,0.0, 0.0, -20.0, 4.0, 0.1, CL_CONTINUOS);
