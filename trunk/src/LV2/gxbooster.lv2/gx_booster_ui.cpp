...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 125-tw, 247 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,160,10);
    cairo_paint (w->crb);
...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.6, 0.4, 0.3, 1.0);
    ...
    pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
    cairo_pattern_add_color_stop_rgba (pat, 0,  0.7, 0.5, 0.3, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.25,  0.6, 0.4, 0.2, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.15, 0.15, 0.15, 1.0);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 250, 266);
    ui->win->label = "GxBooster";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], T_LEVEL,"Treble", ui,20, 30, 100, 125);
    set_adjustment(ui->widget[0]->adj,1.0, 1.0, 0.5, 20.0, 0.1, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], B_LEVEL,"Bass", ui,130, 30, 100, 125);
    set_adjustment(ui->widget[1]->adj,1.0, 1.0, 0.5, 20.0, 0.1, CL_CONTINUOS);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 250, 266);
    }
