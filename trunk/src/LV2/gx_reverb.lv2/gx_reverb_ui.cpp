...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        /*bg */       { 0.00, 0.15, 0.30, 1.00},
        /*base */     { 0.00, 0.15, 0.30, 1.00},
    ...
    main->color_scheme->prelight = (Colors) {
        /*fg */       { 1.0, 1.0, 1.0, 1.0},
        /*bg */       { 0.21, 0.15, 0.30, 1.00},
        /*base */     { 0.21, 0.15, 0.30, 1.00},
...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.24, 0.24, 0.24, 1.0);
    ...
    pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
    cairo_pattern_add_color_stop_rgba (pat, 0,  0.24, 0.24, 0.24, 1.0);
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 550, 180);
    ui->win->parent_struct = ui;
    ui->win->label = "GxReverb-Stereo";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], ROOMSIZE,"Roomsize", ui,40, 30, 100, 115);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 1.0, 0.001, CL_CONTINUOS);
    adj_set_scale(ui->widget[0]->adj, 5.0);

    ui->widget[1] = add_my_knob(ui->widget[1], DAMP,"Damping", ui,160, 30, 100, 115);
    set_adjustment(ui->widget[1]->adj,0.5, 0.5, 0.0, 1.0, 0.001, CL_CONTINUOS);
    adj_set_scale(ui->widget[1]->adj, 5.0);

    ui->widget[2] = add_my_knob(ui->widget[2], LFOFREQ,"Lfo", ui,300, 30, 60, 75);
    ...
    ui->widget[3] = add_my_combobox(ui->widget[3], INVERT, "Mode", model, len, 0, ui, 260, 110, 140, 30);

    ui->widget[4] = add_my_knob(ui->widget[4], WET_DRY,"Dry/Wet", ui,410, 30, 100, 115);
    set_adjustment(ui->widget[4]->adj,50.0, 50.0, 0.0, 100.0, 0.1, CL_CONTINUOS);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 550, 180);
    }
