...
private:
  ...
  int32_t                      prio;
  TonestackMono                ts;
  run_tonestack_feedback           _t_ptr;
  GxAmpFeedBack                    amplifier;
  run_amp_feedback                 _a_ptr;
...
public:
  ...
  void set_tubesel_feedback(const LV2_Descriptor* descriptor);
  inline void run_dsp_feedback(uint32_t n_samples);
  void connect_feedback(uint32_t port,void* data);
  inline void init_dsp_feedback(uint32_t rate, uint32_t bufsize_);
  inline void do_work_feedback();
  inline void connect_all_feedback_ports(uint32_t port, void* data);
  // constructor
  GxPluginFeedBack() :
    output(NULL),
    input(NULL),
...
#include "gx_amp_feedback.cc"

// plugin stuff

void GxPluginFeedBack::do_work_feedback()
{
  if (cab_changed())
    {
...
void GxPluginFeedBack::set_tubesel_feedback(const LV2_Descriptor*     descriptor)
{
  if (strcmp("http://guitarix.sourceforge.net/plugins/gxamp#12ax7f",descriptor->URI)== 0)
    {
      printf("12ax7 feedback\n");
      _a_ptr = &GxAmpFeedBack::run_12ax7_f;
      _t_ptr = &TonestackMono::run_bogner;
      cabconv.cab_count = cab_data_AC30.ir_count;
      cabconv.cab_sr = cab_data_AC30.ir_sr;
      cabconv.cab_data = cab_data_AC30.ir_data;
      tubesel  = 1;
    }
  else if (strcmp("http://guitarix.sourceforge.net/plugins/gxamp#12AT7f",descriptor->URI)== 0)
    {
      printf("12AT7 feedback\n");
      _a_ptr = &GxAmpFeedBack::run_12AT7_f;
      _t_ptr = &TonestackMono::run_jcm2000;
      cabconv.cab_count = cab_data_Marshall.ir_count;
      cabconv.cab_sr = cab_data_Marshall.ir_sr;
      cabconv.cab_data = cab_data_Marshall.ir_data;
      tubesel  = 2;
    }
  else if (strcmp("http://guitarix.sourceforge.net/plugins/gxamp#6DJ8f",descriptor->URI)== 0)
    {
      printf("6DJ8 feedback\n");
      _a_ptr = &GxAmpFeedBack::run_6DJ8_f;
      _t_ptr = &TonestackMono::run_bassman;
      cabconv.cab_count = cab_data_A2.ir_count;
      cabconv.cab_sr =  cab_data_A2.ir_sr;
      cabconv.cab_data = cab_data_A2.ir_data;
      tubesel  = 3;
    }
/*   else if (strcmp("http://guitarix.sourceforge.net/plugins/gxamp#6V6",descriptor->URI)== 0)
    {
      printf("6V6\n");
      _a_ptr = &GxAmpFeedBack::run_6V6;
...
void GxPluginFeedBack::init_dsp_feedback(uint32_t rate, uint32_t bufsize_)
{
  AVOIDDENORMALS();
...
void GxPluginFeedBack::connect_feedback(uint32_t port,void* data)
{
...
void GxPluginFeedBack::run_dsp_feedback(uint32_t n_samples)
{
  // run dsp
...
void GxPluginFeedBack::connect_all_feedback_ports(uint32_t port, void* data)
{
  connect_feedback(port,data);
...
work(LV2_Handle                  instance,
     ...
     const void*                 data)
{
  GxPluginFeedBack* self = (GxPluginFeedBack*)instance;
  self->do_work_feedback();
            ...
            const LV2_Feature* const* features)
{
  ...
  GxPluginFeedBack *self = new GxPluginFeedBack();
  if (!self)
    {
      return NULL;
    }
  ...
  self->set_tubesel_feedback( descriptor);
  self->init_dsp_feedback((uint32_t)rate, bufsize);
...
{
  GxPluginFeedBack* self = (GxPluginFeedBack*)instance;
  self->connect_all_feedback_ports(port, data);
}
...
{
  GxPluginFeedBack* self = (GxPluginFeedBack*)instance;
  self->run_dsp_feedback(n_samples);
}
