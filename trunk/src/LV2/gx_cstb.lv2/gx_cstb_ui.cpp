...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.0, 0.2, 0.2, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.75,  0.0, 0.18, 0.18, 1.0);
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
    ...
    ui->win->label = "GxColorSound Tonebender";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], ATTACK,"Attack", ui,60, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 0.95, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], LEVEL,"Level", ui,240, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,0.2, 0.2, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], WET_DRY,"Dry/Wet", ui,420, 25, 100, 125);
