...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win->label = "GxMultiBandReverb";
    ...
    iw=0;
    w_pos = 85;
    port = (PortIndex) ROOMSIZE1;
    for (;iw<5;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Roomsize", ui,w_pos, 30, 60, 70);
        set_adjustment(ui->widget[iw]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);
    ...
    w_pos = 85;
    port = (PortIndex) DAMP1;
    for (;iw<10;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Damping", ui,w_pos, 105, 60, 70);
        set_adjustment(ui->widget[iw]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);
        //adj_set_scale(ui->widget[iw]->adj, 2.0);
        port +=1;
        w_pos +=100;
    ...
    w_pos = 85;
    port = (PortIndex) WET_DRY1;
    for (;iw<15;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Dry/Wet", ui,w_pos, 180, 60, 70);
        set_adjustment(ui->widget[iw]->adj,50.0, 50.0, 0.0, 100.0, 1.0, CL_CONTINUOS);
        //adj_set_scale(ui->widget[iw]->adj, 2.0);
        port +=1;
        w_pos +=100;
    }
