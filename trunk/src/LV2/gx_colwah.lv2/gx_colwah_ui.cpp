...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 340-tw, 170 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,580,10);
    cairo_paint (w->crb);
...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.35, 0.13, 0.17, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.75,  0.35, 0.0, 0.0, 1.0);
    ...
    pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
    cairo_pattern_add_color_stop_rgba (pat, 0,  0.35, 0.0, 0.0, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.25,  0.35, 0.13, 0.17, 1.0);
    cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.15, 0.15, 0.15, 1.0);
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 680, 180);
    ui->win->parent_struct = ui;
    ui->win->label = "GxWahwah";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], WAH,"Wah", ui,280, 25, 100, 125);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], FREQ,"Alien Freq", ui,400, 25, 100, 125);
    set_adjustment(ui->widget[1]->adj,24.0, 24.0, 24.0, 360.0, 1.0, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], WET_DRY,"Dry/Wet", ui,520, 25, 100, 125);
    ...
    ui->widget[3] = add_my_combobox(ui->widget[3], MODEL, "Model", model, len, 0, ui, 60, 40, 170, 30);

    const char* mode[] = {"manual","auto","alien"};
    len = sizeof(mode) / sizeof(mode[0]);
    ui->widget[4] = add_my_combobox(ui->widget[4], MODE, "Mode", mode, len, 0, ui, 60, 100, 170, 30);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 680, 180);
    }
