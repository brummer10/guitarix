...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 305-tw, 310 );
    cairo_show_text_dummy(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,510,10);
    cairo_paint (w->crb);
...
static void check_multi_controller(X11_UI* ui, uint32_t port_index, float value) {
    ...
    switch ((PortIndex)port_index)
        ...
        case CROSSOVER_B1_B2:
            check_value_changed(ui->widget[15]->adj, &v);
            adj_set_value(ui->adj[0], max(adj_get_value(ui->adj[0]), v+0.1));
        break;
        case CROSSOVER_B2_B3:
            adj_set_value(ui->widget[15]->adj, min(adj_get_value(ui->widget[15]->adj), v-0.1));
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 610, 320);
    ui->win->parent_struct = ui;
    ui->win->label = "GxMultiBandDelay";
    ...
    for (;iw<5;iw++) {
        ui->meter_widget[iw] = add_vmeter(ui->win, "Meter", false, w_pos, 40, 20, 210);
    ...
    iw=0;
    w_pos = 85;
    port = (PortIndex) DELAY1;
    for (;iw<5;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"delay", ui,w_pos, 30, 60, 70);
        set_adjustment(ui->widget[iw]->adj,30.0, 30.0, 24.0, 360.0, 1.0, CL_CONTINUOS);
    ...
    w_pos = 85;
    port = (PortIndex) FEEDBACK1;
    for (;iw<10;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Feedback", ui,w_pos, 105, 60, 70);
        set_adjustment(ui->widget[iw]->adj,50.0, 50.0, 1.0, 100.0, 1.0, CL_CONTINUOS);
        //adj_set_scale(ui->widget[iw]->adj, 2.0);
        port +=1;
        w_pos +=100;
    }
    ...
    w_pos = 85;
    port = (PortIndex) GAIN1;
    for (;iw<15;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Gain", ui,w_pos, 180, 60, 70);
        set_adjustment(ui->widget[iw]->adj,1.0, 1.0, -40.0, 2.0, 0.5, CL_CONTINUOS);
        //adj_set_scale(ui->widget[iw]->adj, 2.0);
        port +=1;
        w_pos +=100;
    }

    ui->widget[15] = add_my_slider(ui->widget[15], CROSSOVER_B1_B2,"Crossover ",ui, 60,270, 480, 20);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 610, 320);
    }
