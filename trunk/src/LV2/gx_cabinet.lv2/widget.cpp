...
plug_name(plugname)
{
  ...
  m_hbox2_.pack_start(c_selector,Gtk::PACK_EXPAND_PADDING);
  make_controller_box(&m_vbox7, "level", 0.5, 5, 0.5, CLevel);
  make_controller_box(&m_vbox5, "bass", -1e+01, 1e+01, 0.5, CBass);
  ...
  m_vbox_.pack_start(m_hbox2_,Gtk::PACK_SHRINK);
...
void Widget::set_sensitive_state(float state)
{
  ...
  m_smallknob2.set_sensitive(set);
  m_smallknob3.set_sensitive(set);
