...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 255-tw, 230 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,410,10);
    cairo_paint (w->crb);
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 510, 240);
    ...
    set_adjustment(ui->widget[1]->adj,1.0, 1.0, -3.0, 100.0, 1.0, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], TONE,"Tone", ui,260, 25, 100, 125);
    set_adjustment(ui->widget[2]->adj,0.5, 0.5, 0.0, 1.0, 0.01, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], OUTPUT,"Output", ui,370, 25, 100, 125);
    set_adjustment(ui->widget[3]->adj,100.0, 100.0, 10.0, 100.0, 1.0, CL_CONTINUOS);

    ui->widget[4] = add_hmeter(ui->win, "Meter", true, 50, 160, 400, 20);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 510, 240);
    }
...
static void port_event(LV2UI_Handle handle, uint32_t port_index,
    ...
    for (;i<CONTROLS;i++) {
        if (port_index == (uint32_t)ui->widget[i]->data) {
            ...
            if (port_index == V1) value = power2db(ui->widget[4], 20.*log10(max(ui->db_zero,value)));
