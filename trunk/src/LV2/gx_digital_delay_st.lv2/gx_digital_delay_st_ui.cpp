...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        .bg =       { 0.00, 0.24, 0.06, 1.00},
        .base =     { 0.00, 0.24, 0.06, 1.00},
    ...
    main->color_scheme->prelight = (Colors) {
        .fg =       { 1.0, 1.0, 1.0, 1.0},
        .bg =       { 0.00, 0.24, 0.21, 1.00},
        .base =     { 0.00, 0.24, 0.21, 1.00},
...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 470-tw, 170 );
...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.15, 0.25, 0.15, 1.00);
    cairo_pattern_add_color_stop_rgba (pat, 0.75,  0.12, 0.2, 0.12, 1.00);
    cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.1, 0.1, 0.15, 1.0);
    ...
    pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
    cairo_pattern_add_color_stop_rgba (pat, 0,  0.00, 0.14, 0.0, 1.00);
    cairo_pattern_add_color_stop_rgba (pat, 0.25,  0.2, 0.24, 0.2, 1.00);
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->widget[6] = add_my_combobox(ui->widget[6], MODE, "Mode", model, len, 0, ui, 390, 110, 160, 30);
    ...
    ui->widget[7] = add_my_combobox(ui->widget[7], NOTES, "Notes",notes, len, 0, ui, 40, 50, 160, 30);

    const char* mode[] = {"BPM free scale","BPM host sync"};
    len = sizeof(mode) / sizeof(mode[0]);
    ui->widget[8] = add_my_combobox(ui->widget[8], SYNC, "BPM SYNC",mode, len, 0, ui, 40, 90, 160, 30);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 780, 180);
    }
