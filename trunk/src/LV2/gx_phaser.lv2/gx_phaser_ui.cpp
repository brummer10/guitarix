...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        .bg =       { 0.13, 0.12, 0.51, 1.00},
        .base =     { 0.13, 0.12, 0.51, 1.00},
    ...
    main->color_scheme->prelight = (Colors) {
        .fg =       { 1.0, 1.0, 1.0, 1.0},
        .bg =       { 0.22, 0.12, 0.00, 1.00},
        .base =     { 0.22, 0.12, 0.00, 1.00},
    ...
    main->color_scheme->selected = (Colors) {
        .fg =       { 0.9, 0.9, 0.9, 1.0},
        .bg =       { 0.61, 0.12, 0.00, 1.00},
        .base =     { 0.61, 0.12, 0.00, 1.00},
...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 130-tw, 285 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,150,15);
    cairo_paint (w->crb);
...
static LV2UI_Handle instantiate(const struct LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 260, 300);
    ui->win->parent_struct = ui;
    ui->win->label = "GxPhaser";
    widget_get_png(ui->win, LDVAR(guitarix_png));
    ui->screw = surface_get_png(ui->win, ui->screw, LDVAR(screw_png));
    // connect the expose func
    ui->win->func.expose_callback = draw_window;
    // create knob widgets
    ui->widget[0] = add_my_knob(ui->widget[0], SPEED,"Speed", ui,30, 40, 80, 105);
    set_adjustment(ui->widget[0]->adj,0.5, 0.5, 0.0, 10.0, 0.1, CL_CONTINUOS);

    ui->widget[1] = add_my_knob(ui->widget[1], LEVEL,"Level", ui,150, 40, 80, 105);
    set_adjustment(ui->widget[1]->adj,0.0, 0.0, -60.0, 10.0, 0.1, CL_CONTINUOS);
    adj_set_scale(ui->widget[1]->adj, 2.0);

    ui->widget[2] = add_my_knob(ui->widget[2], WET_DRY,"Dry/Wet", ui,100, 120, 60, 75);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 260, 300);
    }
