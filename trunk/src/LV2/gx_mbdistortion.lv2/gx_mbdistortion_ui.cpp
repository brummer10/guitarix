...
static void check_multi_controller(X11_UI* ui, uint32_t port_index, float value) {
    ...
    switch ((PortIndex)port_index)
        ...
        case CROSSOVER_B2_B3:
            adj_set_value(ui->widget[11]->adj, min(adj_get_value(ui->widget[11]->adj), v-0.1));
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win->label = "GxMultiBandDistortion";
    ...
    iw=0;
    w_pos = 85;
    port = (PortIndex) DRIVE1;
    for (;iw<5;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Drive", ui,w_pos, 40, 60, 70);
        set_adjustment(ui->widget[iw]->adj,0.0, 0.0, 0.0, 1.0, 0.001, CL_CONTINUOS);
        adj_set_scale(ui->widget[iw]->adj, 5.0);
        port +=1;
        w_pos +=100;
    }

    w_pos = 85;
    port = (PortIndex) OFFSET1;
    for (;iw<10;iw++) {
        ui->widget[iw] = add_my_knob(ui->widget[iw], (PortIndex)port,"Offset", ui,w_pos, 115, 60, 70);
        set_adjustment(ui->widget[iw]->adj,0.0, 0.0, 0.0, 0.5, 0.001, CL_CONTINUOS);
        adj_set_scale(ui->widget[iw]->adj, 5.0);
        port +=1;
        w_pos +=100;
    }

    ui->widget[iw] = add_my_knob(ui->widget[iw], GAIN,"Gain", ui,570, 60, 100, 115);
    set_adjustment(ui->widget[iw]->adj,1.0, 1.0, -40.0, 4.0, 0.5, CL_CONTINUOS);
    //adj_set_scale(ui->widget[iw]->adj, 2.0);

    ui->widget[11] = add_my_slider(ui->widget[11], CROSSOVER_B1_B2,"Crossover ",ui, 60,205, 480, 20);
