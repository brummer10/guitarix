...
static void set_my_theme(Xputty *main) {
    main->color_scheme->normal = (Colors) {
        ...
        /*bg */       { 0.09, 0.09, 0.16, 1.00},
        /*base */     { 0.09, 0.09, 0.16, 1.00},
    ...
    main->color_scheme->prelight = (Colors) {
        /*fg */       { 1.0, 1.0, 1.0, 1.0},
        /*bg */       { 0.13, 0.13, 0.24, 1.00},
        /*base */     { 0.13, 0.13, 0.24, 1.00},
...
static void draw_window(void *w_, void* user_data) {
    ...
    widget_set_scale(w);
    cairo_move_to (w->crb, 295-tw, 170 );
    cairo_show_text(w->crb, w->label);
    cairo_new_path (w->crb);
    cairo_scale (w->crb, 0.95, 0.95);
    cairo_set_source_surface (w->crb, w->image,490,10);
    cairo_paint (w->crb);
...
static void draw_my_knob(void *w_, void* user_data) {
    ...
    cairo_pattern_add_color_stop_rgba (pat, 1,  0.25, 0.25, 0.25, 1.00);
    cairo_pattern_add_color_stop_rgba (pat, 0.85,  0.2, 0.2, 0.2, 1.00);
    ...
    pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
    cairo_pattern_add_color_stop_rgba (pat, 0,  0.25, 0.25, 0.25, 1.00);
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
    ...
    ui->win = create_window(&ui->main, (Window)ui->parentXwindow, 0, 0, 590, 180);
    ui->win->parent_struct = ui;
    ui->win->label = "Gxjcm800pre";
    ...
    set_adjustment(ui->widget[1]->adj,0.5, 0.5, 0.0, 1.0, 0.005, CL_CONTINUOS);

    ui->widget[2] = add_my_knob(ui->widget[2], MIDDLE,"Mid", ui,220, 45, 60, 85);
    set_adjustment(ui->widget[2]->adj,0.5, 0.5, 0.0, 1.0, 0.005, CL_CONTINUOS);

    ui->widget[3] = add_my_knob(ui->widget[3], TREBLE,"Treble", ui,290, 45, 60, 85);
    set_adjustment(ui->widget[3]->adj,0.5, 0.5, 0.0, 1.0, 0.005, CL_CONTINUOS);
    ...
    *widget = (void*)ui->win->widget;
    ...
    if (resize){
        ui->resize = resize;
        resize->ui_resize(resize->handle, 590, 180);
    }
