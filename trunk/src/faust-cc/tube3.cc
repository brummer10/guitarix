...
void compute(int count, float *input0, float *input1, float *output0, float *output1)
{
	...
	for (int i=0; i<count; i++) {
		...
		output1[i] = (FAUSTFLOAT)(0 - (fSlow8 * (((int((fabsf(fTemp13) > 0.0001f)))?((int((fTemp14 < -50)))?(expf(fTemp14) * fTemp15):(fTemp14 / (1 - expf(fTemp15)))):(1 + (fSlow1 * (fTemp12 * (0.5f + (fSlow6 * fTemp12)))))) - fSlow5)));
	}
}
