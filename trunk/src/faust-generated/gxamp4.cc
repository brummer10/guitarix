...
static void compute(int count, float *input0, float *output0)
{
	...
	for (int i=0; i<count; i++) {
		...
		fRec0[0] = ((fConst68 * ((fConst11 * fRec1[1]) + (fConst0 * fRec1[0]))) + (fConst2 * fRec0[1]));
		...
		fRec35[0] = ((fConst68 * ((fConst11 * fRec36[1]) + (fConst0 * fRec36[0]))) + (fConst2 * fRec35[1]));
...
PluginDef plugin = {
    PLUGINDEF_VERSION,
    0,   // flags
    "pre 12AU7/ master 6V6",  // id
    N_("pre 12AU7/ master 6V6"),  // name
    0,  // groups
    compute,  // mono_audio
    0,  // stereo_audio
    init,  // set_samplerate
