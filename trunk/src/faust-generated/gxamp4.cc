// generated from file '../src/faust/gxamp4.dsp' by dsp2cc:
#include "valve.h"
namespace gxamp4 {
static double 	fConst0;
static double 	fConst1;
static double 	fConst2;
static double 	fConst3;
static double 	fConst4;
static double 	fConst5;
static double 	fConst6;
static double 	fRec2[2];
static FAUSTFLOAT&	fslider0 = get_alias("amp2.stage2.gain1");
static double 	fRec3[2];
static FAUSTFLOAT&	fslider1 = get_alias("gxdistortion.wet_dry");
static double 	fRec5[2];
static FAUSTFLOAT&	fslider2 = get_alias("amp2.stage1.Pregain");
static double 	fRec13[2];
static double 	fConst7;
static double 	fConst8;
static double 	fConst9;
static double 	fConst10;
static double 	fRec14[2];
static double 	fRec12[3];
static double 	fConst11;
static double 	fConst12;
static double 	fRec11[2];
static double 	fConst13;
static double 	fRec10[2];
static double 	fConst14;
static double 	fConst15;
static double 	fConst16;
static double 	fConst17;
static double 	fRec15[2];
static double 	fRec9[3];
static double 	fRec8[2];
static double 	fConst18;
static double 	fConst19;
static double 	fConst20;
static double 	fConst21;
static double 	fRec16[2];
static double 	fRec7[3];
static double 	fRec6[2];
static FAUSTFLOAT&	fslider3 = get_alias("gxdistortion.drive");
static double 	fConst22;
static double 	fConst23;
static double 	fConst24;
static double 	fConst25;
static double 	fConst26;
static double 	fConst27;
static double 	fConst28;
static double 	fConst29;
static double 	fConst30;
static double 	fConst31;
static double 	fConst32;
static double 	fConst33;
static double 	fConst34;
static double 	fConst35;
static double 	fConst36;
static double 	fConst37;
static double 	fConst38;
static double 	fConst39;
static double 	fConst40;
static double 	fConst41;
static double 	fConst42;
static double 	fConst43;
static double 	fConst44;
static double 	fConst45;
static double 	fConst46;
static double 	fConst47;
static double 	fConst48;
static double 	fRec23[3];
static double 	fVec0[2];
static double 	fConst49;
static double 	fRec22[2];
static double 	fConst50;
static double 	fConst51;
static double 	fRec21[2];
static double 	fRec20[3];
static double 	fConst52;
static double 	fRec19[3];
static double 	fRec18[3];
static double 	fConst53;
static double 	fConst54;
static double 	fConst55;
static double 	fConst56;
static double 	fConst57;
static double 	fConst58;
static double 	fConst59;
static double 	fConst60;
static double 	fConst61;
static double 	fRec29[2];
static double 	fRec28[3];
static double 	fVec1[2];
static double 	fConst62;
static double 	fRec27[2];
static double 	fRec26[3];
static double 	fVec2[2];
static double 	fConst63;
static double 	fConst64;
static double 	fRec25[2];
static double 	fRec24[3];
static double 	fConst65;
static double 	fRec31[2];
static double 	fRec30[3];
static double 	fConst66;
static double 	fRec34[2];
static double 	fRec33[3];
static double 	fConst67;
static double 	fRec32[3];
static double 	fVec3[2];
static double 	fConst68;
static double 	fRec17[2];
static double 	fVec4[2];
static double 	fRec4[2];
static double 	fRec1[3];
static double 	fConst69;
static double 	fRec0[2];
static double 	fRec37[2];
static double 	fRec36[3];
static double 	fRec35[2];
static int	fSamplingFreq;

static void init(int samplingFreq)
{
	fSamplingFreq = samplingFreq;
	fConst0 = (1.0 / tan((97.38937226128358 / fSamplingFreq)));
	fConst1 = (1 + fConst0);
	fConst2 = (0 - ((1 - fConst0) / fConst1));
	fConst3 = (1.0 / tan((20517.741620594938 / fSamplingFreq)));
	fConst4 = (1 + fConst3);
	fConst5 = (0 - ((1 - fConst3) / fConst4));
	fConst6 = (0.0082 / fConst4);
	for (int i=0; i<2; i++) fRec2[i] = 0;
	for (int i=0; i<2; i++) fRec3[i] = 0;
	for (int i=0; i<2; i++) fRec5[i] = 0;
	for (int i=0; i<2; i++) fRec13[i] = 0;
	fConst7 = (1.0 / tan((270.1769682087222 / fSamplingFreq)));
	fConst8 = (1 + fConst7);
	fConst9 = (0 - ((1 - fConst7) / fConst8));
	fConst10 = (0.027 / fConst8);
	for (int i=0; i<2; i++) fRec14[i] = 0;
	for (int i=0; i<3; i++) fRec12[i] = 0;
	fConst11 = (0 - fConst0);
	fConst12 = (0.1 / fConst1);
	for (int i=0; i<2; i++) fRec11[i] = 0;
	fConst13 = (1.0 / fConst4);
	for (int i=0; i<2; i++) fRec10[i] = 0;
	fConst14 = (1.0 / tan((414.6902302738527 / fSamplingFreq)));
	fConst15 = (1 + fConst14);
	fConst16 = (0 - ((1 - fConst14) / fConst15));
	fConst17 = (0.015 / fConst15);
	for (int i=0; i<2; i++) fRec15[i] = 0;
	for (int i=0; i<3; i++) fRec9[i] = 0;
	for (int i=0; i<2; i++) fRec8[i] = 0;
	fConst18 = (1.0 / tan((609.4689747964198 / fSamplingFreq)));
	fConst19 = (1 + fConst18);
	fConst20 = (0 - ((1 - fConst18) / fConst19));
	fConst21 = (0.0082 / fConst19);
	for (int i=0; i<2; i++) fRec16[i] = 0;
	for (int i=0; i<3; i++) fRec7[i] = 0;
	for (int i=0; i<2; i++) fRec6[i] = 0;
	fConst22 = tan((942.4777960769379 / fSamplingFreq));
	fConst23 = (1.0 / faustpower<2>(fConst22));
	fConst24 = (2 * (1 - fConst23));
	fConst25 = (1.0 / fConst22);
	fConst26 = (1 + ((fConst25 - 1.0) / fConst22));
	fConst27 = (1.0 / (1 + ((1.0 + fConst25) / fConst22)));
	fConst28 = tan((3769.9111843077517 / fSamplingFreq));
	fConst29 = (1.0 / faustpower<2>(fConst28));
	fConst30 = (2 * (1 - fConst29));
	fConst31 = (1.0 / fConst28);
	fConst32 = (1 + ((fConst31 - 1.0) / fConst28));
	fConst33 = (1.0 / (1 + ((1.0 + fConst31) / fConst28)));
	fConst34 = tan((10053.096491487338 / fSamplingFreq));
	fConst35 = (1.0 / faustpower<2>(fConst34));
	fConst36 = (2 * (1 - fConst35));
	fConst37 = (1.0 / fConst34);
	fConst38 = (1 + ((fConst37 - 1.0000000000000004) / fConst34));
	fConst39 = (1 + ((1.0000000000000004 + fConst37) / fConst34));
	fConst40 = (1.0 / fConst39);
	fConst41 = (1 + fConst37);
	fConst42 = (0 - ((1 - fConst37) / fConst41));
	fConst43 = tan((47123.8898038469 / fSamplingFreq));
	fConst44 = (2 * (1 - (1.0 / faustpower<2>(fConst43))));
	fConst45 = (1.0 / fConst43);
	fConst46 = (1 + ((fConst45 - 1.414213562373095) / fConst43));
	fConst47 = (1 + ((1.414213562373095 + fConst45) / fConst43));
	fConst48 = (1.0 / fConst47);
	for (int i=0; i<3; i++) fRec23[i] = 0;
	for (int i=0; i<2; i++) fVec0[i] = 0;
	fConst49 = (1.0 / (fConst1 * fConst47));
	for (int i=0; i<2; i++) fRec22[i] = 0;
	fConst50 = (0 - fConst37);
	fConst51 = (1.0 / fConst41);
	for (int i=0; i<2; i++) fRec21[i] = 0;
	for (int i=0; i<3; i++) fRec20[i] = 0;
	fConst52 = (2 * (0 - fConst35));
	for (int i=0; i<3; i++) fRec19[i] = 0;
	for (int i=0; i<3; i++) fRec18[i] = 0;
	fConst53 = (1 + ((fConst25 - 1.0000000000000004) / fConst22));
	fConst54 = (1.0 / (1 + ((fConst25 + 1.0000000000000004) / fConst22)));
	fConst55 = (1 + fConst25);
	fConst56 = (0 - ((1 - fConst25) / fConst55));
	fConst57 = (1 + ((fConst31 - 1.0000000000000004) / fConst28));
	fConst58 = (1 + ((fConst31 + 1.0000000000000004) / fConst28));
	fConst59 = (1.0 / fConst58);
	fConst60 = (1 + fConst31);
	fConst61 = (0 - ((1 - fConst31) / fConst60));
	for (int i=0; i<2; i++) fRec29[i] = 0;
	for (int i=0; i<3; i++) fRec28[i] = 0;
	for (int i=0; i<2; i++) fVec1[i] = 0;
	fConst62 = (1.0 / (fConst60 * fConst39));
	for (int i=0; i<2; i++) fRec27[i] = 0;
	for (int i=0; i<3; i++) fRec26[i] = 0;
	for (int i=0; i<2; i++) fVec2[i] = 0;
	fConst63 = (0 - fConst25);
	fConst64 = (1.0 / (fConst55 * fConst58));
	for (int i=0; i<2; i++) fRec25[i] = 0;
	for (int i=0; i<3; i++) fRec24[i] = 0;
	fConst65 = (2 * (0 - fConst23));
	for (int i=0; i<2; i++) fRec31[i] = 0;
	for (int i=0; i<3; i++) fRec30[i] = 0;
	fConst66 = (0 - fConst31);
	for (int i=0; i<2; i++) fRec34[i] = 0;
	for (int i=0; i<3; i++) fRec33[i] = 0;
	fConst67 = (2 * (0 - fConst29));
	for (int i=0; i<3; i++) fRec32[i] = 0;
	for (int i=0; i<2; i++) fVec3[i] = 0;
	fConst68 = (1.5848931924611136 / fConst4);
	for (int i=0; i<2; i++) fRec17[i] = 0;
	for (int i=0; i<2; i++) fVec4[i] = 0;
	for (int i=0; i<2; i++) fRec4[i] = 0;
	for (int i=0; i<3; i++) fRec1[i] = 0;
	fConst69 = (0.025 / fConst1);
	for (int i=0; i<2; i++) fRec0[i] = 0;
	for (int i=0; i<2; i++) fRec37[i] = 0;
	for (int i=0; i<3; i++) fRec36[i] = 0;
	for (int i=0; i<2; i++) fRec35[i] = 0;
}

void compute(int count, float *input0, float *output0)
{
	double 	fSlow0 = (0.0010000000000000009 * pow(10,(0.05 * fslider0)));
	double 	fSlow1 = (1.000000000000001e-05 * fslider1);
	double 	fSlow2 = (0.0010000000000000009 * pow(10,(0.05 * fslider2)));
	double 	fSlow3 = fslider3;
	double 	fSlow4 = (1.25 * fSlow3);
	double 	fSlow5 = (2 * (fSlow3 - 0.5));
	double 	fSlow6 = (1 - max(0, fSlow5));
	double 	fSlow7 = pow(1e+01,(0.8 * fSlow3));
	double 	fSlow8 = (fConst54 * pow(1e+01,(2 * fSlow3)));
	double 	fSlow9 = (fConst54 * pow(1e+01,(0.9 * fSlow3)));
	double 	fSlow10 = pow(1e+01,(1.2 * fSlow3));
	double 	fSlow11 = (1 - max(0, (0 - fSlow5)));
	for (int i=0; i<count; i++) {
		fRec2[0] = ((fConst6 * (fRec1[1] + fRec1[2])) + (fConst5 * fRec2[1]));
		fRec3[0] = (fSlow0 + (0.999 * fRec3[1]));
		fRec5[0] = (fSlow1 + (0.999 * fRec5[1]));
		double fTemp0 = (1 - fRec5[0]);
		fRec13[0] = (fSlow2 + (0.999 * fRec13[1]));
		fRec14[0] = ((fConst10 * (fRec12[1] + fRec12[2])) + (fConst9 * fRec14[1]));
		fRec12[0] = (Ftube(TUBE_TABLE_12AU7_68k, ((fRec14[0] + ((double)input0[i] * fRec13[0])) - 1.25724)) - 83.43555555555557);
		fRec11[0] = ((fConst12 * ((fConst11 * fRec12[1]) + (fConst0 * fRec12[0]))) + (fConst2 * fRec11[1]));
		fRec10[0] = ((fConst13 * (fRec11[0] + fRec11[1])) + (fConst5 * fRec10[1]));
		fRec15[0] = ((fConst17 * (fRec9[1] + fRec9[2])) + (fConst16 * fRec15[1]));
		fRec9[0] = (Ftube(TUBE_TABLE_12AU7_250k, ((fRec15[0] + fRec10[0]) - 0.776162)) - 78.25586666666666);
		fRec8[0] = ((fConst12 * ((fConst11 * fRec9[1]) + (fConst0 * fRec9[0]))) + (fConst2 * fRec8[1]));
		fRec16[0] = ((fConst21 * (fRec7[1] + fRec7[2])) + (fConst20 * fRec16[1]));
		fRec7[0] = (Ftube(TUBE_TABLE_12AU7_250k, ((fRec16[0] + fRec8[0]) - 0.445487)) - 75.67231707317073);
		fRec6[0] = ((fConst12 * ((fConst11 * fRec7[1]) + (fConst0 * fRec7[0]))) + (fConst2 * fRec6[1]));
		double fTemp1 = (fConst24 * fRec18[1]);
		double fTemp2 = (fConst30 * fRec19[1]);
		fRec23[0] = ((fRec6[0] * fRec5[0]) - (fConst48 * ((fConst46 * fRec23[2]) + (fConst44 * fRec23[1]))));
		double fTemp3 = (fRec23[2] + (fRec23[0] + (2 * fRec23[1])));
		fVec0[0] = fTemp3;
		fRec22[0] = ((fConst49 * ((fConst11 * fVec0[1]) + (fConst0 * fVec0[0]))) + (fConst2 * fRec22[1]));
		fRec21[0] = ((fConst51 * ((fConst50 * fRec22[1]) + (fConst37 * fRec22[0]))) + (fConst42 * fRec21[1]));
		fRec20[0] = (fRec21[0] - (fConst40 * ((fConst38 * fRec20[2]) + (fConst36 * fRec20[1]))));
		fRec19[0] = ((fConst40 * (((fConst35 * fRec20[0]) + (fConst52 * fRec20[1])) + (fConst35 * fRec20[2]))) - (fConst33 * ((fConst32 * fRec19[2]) + fTemp2)));
		fRec18[0] = ((fRec19[2] + (fConst33 * (fTemp2 + (fConst32 * fRec19[0])))) - (fConst27 * ((fConst26 * fRec18[2]) + fTemp1)));
		double fTemp4 = max(-1, min(1, (fSlow7 * (fRec18[2] + (fConst27 * (fTemp1 + (fConst26 * fRec18[0])))))));
		fRec29[0] = ((fConst51 * (fRec22[0] + fRec22[1])) + (fConst42 * fRec29[1]));
		fRec28[0] = (fRec29[0] - (fConst40 * ((fConst38 * fRec28[2]) + (fConst36 * fRec28[1]))));
		double fTemp5 = (fRec28[2] + (fRec28[0] + (2 * fRec28[1])));
		fVec1[0] = fTemp5;
		fRec27[0] = ((fConst62 * (fVec1[0] + fVec1[1])) + (fConst61 * fRec27[1]));
		fRec26[0] = (fRec27[0] - (fConst59 * ((fConst57 * fRec26[2]) + (fConst30 * fRec26[1]))));
		double fTemp6 = (fRec26[2] + (fRec26[0] + (2 * fRec26[1])));
		fVec2[0] = fTemp6;
		fRec25[0] = ((fConst64 * ((fConst63 * fVec2[1]) + (fConst25 * fVec2[0]))) + (fConst56 * fRec25[1]));
		fRec24[0] = (fRec25[0] - (fConst54 * ((fConst53 * fRec24[2]) + (fConst24 * fRec24[1]))));
		double fTemp7 = max(-1, min(1, (fSlow8 * (((fConst23 * fRec24[0]) + (fConst65 * fRec24[1])) + (fConst23 * fRec24[2])))));
		fRec31[0] = ((fConst64 * (fVec2[0] + fVec2[1])) + (fConst56 * fRec31[1]));
		fRec30[0] = (fRec31[0] - (fConst54 * ((fConst53 * fRec30[2]) + (fConst24 * fRec30[1]))));
		double fTemp8 = max(-1, min(1, (fSlow9 * (fRec30[2] + (fRec30[0] + (2 * fRec30[1]))))));
		double fTemp9 = (fConst24 * fRec32[1]);
		fRec34[0] = ((fConst62 * ((fConst66 * fVec1[1]) + (fConst31 * fVec1[0]))) + (fConst61 * fRec34[1]));
		fRec33[0] = (fRec34[0] - (fConst59 * ((fConst57 * fRec33[2]) + (fConst30 * fRec33[1]))));
		fRec32[0] = ((fConst59 * (((fConst29 * fRec33[0]) + (fConst67 * fRec33[1])) + (fConst29 * fRec33[2]))) - (fConst27 * ((fConst26 * fRec32[2]) + fTemp9)));
		double fTemp10 = max(-1, min(1, (fSlow10 * (fRec32[2] + (fConst27 * (fTemp9 + (fConst26 * fRec32[0])))))));
		double fTemp11 = (((fTemp10 * (1 - (0.3333333333333333 * faustpower<2>(fTemp10)))) + ((fTemp8 * (1 - (0.3333333333333333 * faustpower<2>(fTemp8)))) + (0.841395141645195 * (fTemp7 * (1 - (0.3333333333333333 * faustpower<2>(fTemp7))))))) + (0.7943282347242815 * (fTemp4 * (1 - (0.3333333333333333 * faustpower<2>(fTemp4))))));
		fVec3[0] = fTemp11;
		fRec17[0] = ((fConst68 * (fVec3[0] + fVec3[1])) + (fConst5 * fRec17[1]));
		double fTemp12 = ((fSlow11 * fRec17[0]) + (fRec6[0] * ((fSlow6 * fRec5[0]) + (fTemp0 * (1 + (fSlow4 * ((4 - (4 * fabs((fRec6[0] * fTemp0)))) - 1)))))));
		fVec4[0] = fTemp12;
		fRec4[0] = ((fConst13 * (fVec4[0] + fVec4[1])) + (fConst5 * fRec4[1]));
		double fTemp13 = (fRec4[0] * fRec3[0]);
		fRec1[0] = (Ftube(TUBE_TABLE_6V6_68k, ((fTemp13 + fRec2[0]) - 1.13074)) - 112.10487804878048);
		fRec0[0] = ((fConst69 * ((fConst11 * fRec1[1]) + (fConst0 * fRec1[0]))) + (fConst2 * fRec0[1]));
		fRec37[0] = ((fConst6 * (fRec36[1] + fRec36[2])) + (fConst5 * fRec37[1]));
		fRec36[0] = (Ftube(TUBE_TABLE_6V6_250k, ((fRec37[0] + fTemp13) - 1.130462)) - 112.13878048780487);
		fRec35[0] = ((fConst69 * ((fConst11 * fRec36[1]) + (fConst0 * fRec36[0]))) + (fConst2 * fRec35[1]));
		output0[i] = (FAUSTFLOAT)(fRec35[0] + fRec0[0]);
		// post processing
		fRec35[1] = fRec35[0];
		fRec36[2] = fRec36[1]; fRec36[1] = fRec36[0];
		fRec37[1] = fRec37[0];
		fRec0[1] = fRec0[0];
		fRec1[2] = fRec1[1]; fRec1[1] = fRec1[0];
		fRec4[1] = fRec4[0];
		fVec4[1] = fVec4[0];
		fRec17[1] = fRec17[0];
		fVec3[1] = fVec3[0];
		fRec32[2] = fRec32[1]; fRec32[1] = fRec32[0];
		fRec33[2] = fRec33[1]; fRec33[1] = fRec33[0];
		fRec34[1] = fRec34[0];
		fRec30[2] = fRec30[1]; fRec30[1] = fRec30[0];
		fRec31[1] = fRec31[0];
		fRec24[2] = fRec24[1]; fRec24[1] = fRec24[0];
		fRec25[1] = fRec25[0];
		fVec2[1] = fVec2[0];
		fRec26[2] = fRec26[1]; fRec26[1] = fRec26[0];
		fRec27[1] = fRec27[0];
		fVec1[1] = fVec1[0];
		fRec28[2] = fRec28[1]; fRec28[1] = fRec28[0];
		fRec29[1] = fRec29[0];
		fRec18[2] = fRec18[1]; fRec18[1] = fRec18[0];
		fRec19[2] = fRec19[1]; fRec19[1] = fRec19[0];
		fRec20[2] = fRec20[1]; fRec20[1] = fRec20[0];
		fRec21[1] = fRec21[0];
		fRec22[1] = fRec22[0];
		fVec0[1] = fVec0[0];
		fRec23[2] = fRec23[1]; fRec23[1] = fRec23[0];
		fRec6[1] = fRec6[0];
		fRec7[2] = fRec7[1]; fRec7[1] = fRec7[0];
		fRec16[1] = fRec16[0];
		fRec8[1] = fRec8[0];
		fRec9[2] = fRec9[1]; fRec9[1] = fRec9[0];
		fRec15[1] = fRec15[0];
		fRec10[1] = fRec10[0];
		fRec11[1] = fRec11[0];
		fRec12[2] = fRec12[1]; fRec12[1] = fRec12[0];
		fRec14[1] = fRec14[0];
		fRec13[1] = fRec13[0];
		fRec5[1] = fRec5[0];
		fRec3[1] = fRec3[0];
		fRec2[1] = fRec2[0];
	}
}

static struct RegisterParams { RegisterParams(); } RegisterParams;
RegisterParams::RegisterParams()
{
	registerInit("amp2", init);
}

} // end namespace gxamp4
