...
#define fVslider1 (*fVslider1_)
	...
	for (int i = 0; (i < ReCount); i = (i + 1)) {
		...
		fRec1[0] = ((double(buf[i]) * fRec2[0]) - (fConst2 * (((((fConst6 * fRec1[1]) + (fConst8 * fRec1[2])) + (fConst9 * fRec1[3])) + (fConst10 * fRec1[4])) + (fConst11 * fRec1[5]))));
		...
		buf[i] = FAUSTFLOAT((0.80000000000000004 * (fRec0[0] * (int(signbit(double(fTemp0))) ? double(supersonic_negclip(double(fTemp0))) : double(supersonicclip(double(fTemp0)))))));
		fRec0[1] = fRec0[0];
		fRec2[1] = fRec2[0];
...
{
	fVslider0_ = reg.registerFloatVar("poweramp.Gain","","SA","",&fVslider0, 0.0, -20.0, 20.0, 0.10000000000000001, 0);
	fVslider1_ = reg.registerFloatVar("poweramp.Pregain","","SA","",&fVslider1, 0.0, -20.0, 20.0, 0.10000000000000001, 0);
	return 0;
}
