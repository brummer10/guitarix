...
FAUSTFLOAT&	fslider0 = get_alias("crybaby.wet_dry");
int 	IOTA;
int 	iVec0[1024];
int 	iRec2[2];
FAUSTFLOAT&	fslider1 = get_alias("crybaby.wah");
double 	fRec1[2];
FAUSTFLOAT&	fslider2 = get_alias("crybaby.level");
double 	fConst0;
double 	fConst1;
double 	fRec3[2];
double 	fRec4[2];
...
void compute(int count, float *input0, float *output0)
{
	...
	for (int i=0; i<count; i++) {
		...
		fRec0[0] = (0 - (((fRec4[0] * fRec0[2]) + (fRec3[0] * fRec0[1])) - (fSlow3 * (fTemp0 * fRec1[0]))));
		output0[i] = (FAUSTFLOAT)((fSlow4 * (fRec0[0] - fRec0[1])) + (fSlow1 * fTemp0));
		// post processing
