...
inline void Dsp::init(unsigned int samplingFreq)
{
	...
	fConst1 = (3.141592653589793f / iConst0);
	IOTA = 0;
	fConst2 = (1e+01f / iConst0);
	fConst3 = (0 - fConst2);
...
void always_inline Dsp::compute(int count, float *input0, float *input1, float *output0, float *output1)
{
	...
	for (int i=0; i<count; i++) {
		...
		int iTemp4 = int((int(fRec5[0]) & 262143));
		float fTemp5 = (1.0f - fRec4[0]);
		int iTemp6 = int((int(fRec6[0]) & 262143));
		fRec0[0] = (0.01f * ((fSlow19 * ((fRec4[0] * fVec0[(IOTA-iTemp6)&262143]) + (fTemp5 * fVec0[(IOTA-iTemp4)&262143]))) + ((fSlow0 * fTemp0) + (fSlow17 * fTemp1))));
		...
		fVec1[IOTA&262143] = fTemp9;
		fRec7[0] = (0.01f * ((fSlow19 * ((fRec4[0] * fVec1[(IOTA-iTemp6)&262143]) + (fTemp5 * fVec1[(IOTA-iTemp4)&262143]))) + ((fSlow0 * fTemp7) + (fSlow17 * fTemp8))));
...
int Dsp::register_par(const ParamReg& reg)
{
	...
	reg.registerVar("didest.show_always.highpass (hz)","","S",N_("highpass filter frequency in the feddback loop"),&fslider2, 1.2e+02f, 2e+01f, 2e+04f, 1.0f);
	reg.registerVar("didest.show_always.howpass (hz)","","S",N_("lowpass filter frequency in the feddback loop"),&fslider1, 1.2e+04f, 2e+01f, 2e+04f, 1.0f);
