...
#define fVslider2 (*fVslider2_)
	...
	double fSlow9 = 0.0 - 2.0 * (fSlow0 + fSlow4 + -1.0);
	...
	double fSlow15 = fSlow11 * (fSlow14 + (1.0 - fSlow12));
	double fSlow16 = fConst6 * (fSlow11 + 1.0);
	double fSlow17 = (0.0 - 2.0 * fSlow11) * (fSlow11 + fSlow16 + -1.0);
	double fSlow18 = fSlow11 * (fSlow12 + fSlow14 + 1.0);
	...
	double fSlow23 = fSlow22 * std::pow(1e+01, 0.0 - 0.1 * fSlow22);
	for (int i0 = 0; i0 < count; i0 = i0 + 1) {
		double fTemp0 = double(input0[i0]);
		fVec0[0] = fTemp0;
		fRec1[0] = 0.0 - fSlow10 * (fSlow9 * fRec1[1] + fSlow8 * fRec1[2] - fSlow0 * (fSlow6 * fTemp0 + fSlow5 * fVec0[1] + fSlow3 * fVec0[2]));
		fRec0[0] = fSlow21 * (0.0 - (fSlow20 * fRec0[2] + fSlow19 * fRec0[1]) + fSlow18 * fRec1[0] + fSlow17 * fRec1[1] + fSlow15 * fRec1[2]);
...
{
	fVslider2_ = reg.registerFloatVar("cab_st.Level",N_("Level"),"SA",N_("Level"),&fVslider2, 1.0, 0.5, 5.0, 0.5, 0);
	fVslider0_ = reg.registerFloatVar("cab_st.bass",N_("Bass"),"SA",N_("Bass"),&fVslider0, 0.0, -1e+01, 1e+01, 0.5, 0);
	fVslider1_ = reg.registerFloatVar("cab_st.treble",N_("Treble"),"SA",N_("Treble"),&fVslider1, 0.0, -1e+01, 1e+01, 0.5, 0);
	return 0;
}
