...
inline void Dsp::compute(int count, float *input0, float *input1, float *output0, float *output1)
{
	...
	for (int i=0; i<count; i++) {
		...
		output0[i] = (FAUSTFLOAT)(((1 - (fSlow11 * fRec24[0])) * (fRec1 + (fSlow2 * fTemp0))) + (fSlow1 * fTemp0));
		...
		fVec20[IOTA&2047] = (fTemp4 + (fSlow5 * fRec49[0]));
		...
		output1[i] = (FAUSTFLOAT)(((1 - (fSlow11 * (0 - fRec24[0]))) * (fRec27 + (fSlow2 * fTemp3))) + (fSlow1 * fTemp3));
		// post processing
		fRec26[1] = fRec26[0];
		fRec28[1] = fRec28[0];
		fRec30[1] = fRec30[0];
