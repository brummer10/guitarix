...
void always_inline Dsp::compute(int count, float *input0, float *output0)
{
	...
	for (int i=0; i<count; i++) {
		...
		fRec0[0] = (0.01f * ((fSlow23 * ((fRec29[0] * fVec2[(IOTA-int((int(fRec31[0]) & 524287)))&524287]) + ((1.0f - fRec29[0]) * fVec2[(IOTA-int((int(fRec30[0]) & 524287)))&524287]))) + ((fSlow0 * fTemp0) + (1e+02f * fTemp19))));
