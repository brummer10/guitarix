...
void always_inline Dsp::compute(int count, float *input0, float *output0)
{
	...
	for (int i=0; i<count; i++) {
		...
		fRec0[0] = (0.01f * ((fSlow19 * ((fRec4[0] * fVec0[(IOTA-int((int(fRec6[0]) & 524287)))&524287]) + ((1.0f - fRec4[0]) * fVec0[(IOTA-int((int(fRec5[0]) & 524287)))&524287]))) + ((fSlow0 * fTemp0) + (fSlow17 * fTemp1))));
...
{
	reg.registerVar("dide.level","","S",N_("percentage of the delay gain level"),&fslider6, 5e+01f, 0.0f, 1e+02f, 1.0f);
	reg.registerVar("dide.bpm",N_("delay (bpm)"),"S",N_("Delay in Beats per Minute"),&fslider5, 1.2e+02f, 24.0f, 3.6e+02f, 1.0f);
	...
	reg.registerEnumVar("dide.notes",N_("tact"),"S",N_("note setting for bpm"),fslider4_values,&fslider4, 5.0f, 1.0f, 18.0f, 1.0f);
