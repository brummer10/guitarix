...
inline void Dsp::clear_state_f()
{
	...
	for (int l57 = 0; l57 < 131072; l57 = l57 + 1) fVec9[l57] = 0.0;
}
...
void always_inline Dsp::compute(int count, FAUSTFLOAT *input0, FAUSTFLOAT *output0)
{
	...
	double fSlow60 = fConst3 * double(fHslider16);
	double fSlow61 = 0.1 * double(fHslider17);
	double fSlow62 = 0.0010000000000000009 * double(fHslider18);
	double fSlow63 = double(fHslider19);
	double fSlow64 = 1.0 / (fSlow1 * fSlow57);
	...
	for (int i0 = 0; i0 < count; i0 = i0 + 1) {
		...
		fVec6[0] = fTemp40;
		fRec36[0] = -(fSlow59 * (fSlow4 * fRec36[1] - (fTemp40 + fVec6[1])));
		fRec35[0] = fRec36[0] - fSlow58 * (fSlow56 * fRec35[2] + fSlow2 * fRec35[1]);
		double fTemp41 = fSlow58 * (fRec35[2] + fRec35[0] + 2.0 * fRec35[1]);
		fVec7[IOTA0 & 131071] = fTemp41;
		fRec39[0] = fSlow60 + (fRec39[1] - std::floor(fSlow60 + fRec39[1]));
		...
		double fTemp45 = fConst4 * fRec40[0] * (fSlow61 * ((fTemp43 + (1.0 - fTemp42)) * ftbl0mydspSIG0[iTemp44 & 65535] + (fTemp42 - fTemp43) * ftbl0mydspSIG0[(iTemp44 + 1) & 65535]) + 1.0);
		int iTemp46 = int(fTemp45);
		double fTemp47 = std::floor(fTemp45);
		double fTemp48 = fSlow63 * (fTemp41 + fSlow63 * (fVec7[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp46))) & 131071] * (fTemp47 + (1.0 - fTemp45)) + (fTemp45 - fTemp47) * fVec7[(IOTA0 - std::min<int>(65537, std::max<int>(0, iTemp46 + 1))) & 131071]));
		...
		fRec45[0] = -(fSlow59 * (fSlow4 * fRec45[1] - fSlow3 * (fTemp40 - fVec6[1])));
		fRec44[0] = fRec45[0] - fSlow58 * (fSlow56 * fRec44[2] + fSlow2 * fRec44[1]);
		double fTemp51 = 2.0 * fRec44[1];
		double fTemp52 = fSlow64 * (fRec44[2] + (fRec44[0] - fTemp51));
		fVec8[IOTA0 & 131071] = fTemp52;
		fRec46[0] = fSlow65 + (fRec46[1] - std::floor(fSlow65 + fRec46[1]));
		double fTemp53 = 65536.0 * (fRec46[0] - std::floor(fRec46[0]));
		double fTemp54 = std::floor(fTemp53);
		int iTemp55 = int(fTemp54);
		fRec47[0] = fSlow67 + 0.999 * fRec47[1];
		double fTemp56 = fConst4 * fRec47[0] * (fSlow66 * ((fTemp54 + (1.0 - fTemp53)) * ftbl0mydspSIG0[iTemp55 & 65535] + (fTemp53 - fTemp54) * ftbl0mydspSIG0[(iTemp55 + 1) & 65535]) + 1.0);
		int iTemp57 = int(fTemp56);
		int iTemp58 = std::min<int>(65537, std::max<int>(0, iTemp57 + 1));
		double fTemp59 = std::floor(fTemp56);
		double fTemp60 = fTemp56 - fTemp59;
		double fTemp61 = fTemp59 + (1.0 - fTemp56);
		int iTemp62 = std::min<int>(65537, std::max<int>(0, iTemp57));
		double fTemp63 = std::max<double>(fConst1, std::fabs(fSlow68 * (fTemp52 + fSlow68 * (fVec8[(IOTA0 - iTemp62) & 131071] * fTemp61 + fTemp60 * fVec8[(IOTA0 - iTemp58) & 131071]))));
		fRec41[0] = ((iTemp50) ? fTemp63 + fRec41[1] : fTemp63);
		iRec42[0] = ((iTemp50) ? iRec42[1] + 1 : 1);
		fRec43[0] = ((iTemp50) ? fRec43[1] : 0.000244140625 * fRec41[1]);
		fVbargraph4 = FAUSTFLOAT(fRec43[0]);
		double fTemp64 = fSlow64 * (fRec44[0] + fRec44[2] - fTemp51);
		fVec9[IOTA0 & 131071] = fTemp64;
		output0[i0] = FAUSTFLOAT(fSlow68 * (fTemp64 + fSlow68 * (fTemp61 * fVec9[(IOTA0 - iTemp62) & 131071] + fTemp60 * fVec9[(IOTA0 - iTemp58) & 131071])) + fTemp48 + fTemp37 + fTemp25 + fTemp4 + fTemp12);
		fVec0[1] = fVec0[0];
		fRec7[1] = fRec7[0];
		fRec6[2] = fRec6[1];
		fRec6[1] = fRec6[0];
...
int Dsp::register_par(const ParamReg& reg)
{
	...
	reg.registerFloatVar("mbchor.delay1","","S","",&fHslider18, 0.02, 0.0, 0.2, 0.01, 0);
	reg.registerFloatVar("mbchor.delay2","","S","",&fHslider22, 0.04, 0.0, 0.2, 0.01, 0);
	...
	reg.registerFloatVar("mbchor.depth1","","S","",&fHslider17, 0.02, 0.01, 1.0, 0.01, 0);
	reg.registerFloatVar("mbchor.depth2","","S","",&fHslider21, 0.04, 0.01, 1.0, 0.01, 0);
	...
	reg.registerFloatVar("mbchor.freq1","","S",N_("Beats per Minute"),&fHslider16, 3e+01, 24.0, 3.6e+02, 1.0, 0);
	reg.registerFloatVar("mbchor.freq2","","S",N_("Beats per Minute"),&fHslider20, 6e+01, 24.0, 3.6e+02, 1.0, 0);
	...
	reg.registerFloatVar("mbchor.level1","","S","",&fHslider19, 0.5, 0.0, 1.0, 0.01, 0);
	reg.registerFloatVar("mbchor.level2","","S","",&fHslider23, 0.5, 0.0, 1.0, 0.01, 0);
	...
	reg.registerFloatVar("mbchor.v1","","SOLN","",&fVbargraph3, 0, 0.0, 1.0, 0, 0);
	reg.registerFloatVar("mbchor.v2","","SOLN","",&fVbargraph4, 0, 0.0, 1.0, 0, 0);
