...
void always_inline Dsp::compute(int count, FAUSTFLOAT *input0, FAUSTFLOAT *input1, FAUSTFLOAT *output0, FAUSTFLOAT *output1)
{
	...
	double fThen2 = std::exp(0.0 - fConst1 / ((iSlow49) ? 1.0 : fSlow48));
	double fSlow50 = ((iSlow49) ? 0.0 : fThen2);
	double fSlow51 = 1.0 - fSlow50;
	int iSlow52 = std::fabs(fSlow47) < 2.2204460492503131e-16;
	double fThen5 = std::exp(0.0 - fConst1 / ((iSlow52) ? 1.0 : fSlow47));
	double fSlow53 = ((iSlow52) ? 0.0 : fThen5);
	double fSlow54 = double(fHslider9);
	int iSlow55 = std::fabs(fSlow54) < 2.2204460492503131e-16;
	double fThen7 = std::exp(0.0 - fConst1 / ((iSlow55) ? 1.0 : fSlow54));
	double fSlow56 = ((iSlow55) ? 0.0 : fThen7);
	...
	double fThen16 = std::exp(0.0 - fConst1 / ((iSlow73) ? 1.0 : fSlow72));
	double fSlow74 = ((iSlow73) ? 0.0 : fThen16);
	double fSlow75 = 1.0 - fSlow74;
	int iSlow76 = std::fabs(fSlow71) < 2.2204460492503131e-16;
	double fThen19 = std::exp(0.0 - fConst1 / ((iSlow76) ? 1.0 : fSlow71));
	double fSlow77 = ((iSlow76) ? 0.0 : fThen19);
	double fSlow78 = double(fHslider15);
	int iSlow79 = std::fabs(fSlow78) < 2.2204460492503131e-16;
	double fThen21 = std::exp(0.0 - fConst1 / ((iSlow79) ? 1.0 : fSlow78));
	double fSlow80 = ((iSlow79) ? 0.0 : fThen21);
	...
	double fThen30 = std::exp(0.0 - fConst1 / ((iSlow97) ? 1.0 : fSlow96));
	double fSlow98 = ((iSlow97) ? 0.0 : fThen30);
	double fSlow99 = 1.0 - fSlow98;
	int iSlow100 = std::fabs(fSlow95) < 2.2204460492503131e-16;
	double fThen33 = std::exp(0.0 - fConst1 / ((iSlow100) ? 1.0 : fSlow95));
	double fSlow101 = ((iSlow100) ? 0.0 : fThen33);
	double fSlow102 = double(fHslider21);
	int iSlow103 = std::fabs(fSlow102) < 2.2204460492503131e-16;
	double fThen35 = std::exp(0.0 - fConst1 / ((iSlow103) ? 1.0 : fSlow102));
	double fSlow104 = ((iSlow103) ? 0.0 : fThen35);
	...
	double fThen44 = std::exp(0.0 - fConst1 / ((iSlow120) ? 1.0 : fSlow119));
	double fSlow121 = ((iSlow120) ? 0.0 : fThen44);
	double fSlow122 = 1.0 - fSlow121;
	int iSlow123 = std::fabs(fSlow118) < 2.2204460492503131e-16;
	double fThen47 = std::exp(0.0 - fConst1 / ((iSlow123) ? 1.0 : fSlow118));
	double fSlow124 = ((iSlow123) ? 0.0 : fThen47);
	double fSlow125 = double(fHslider27);
	int iSlow126 = std::fabs(fSlow125) < 2.2204460492503131e-16;
	double fThen49 = std::exp(0.0 - fConst1 / ((iSlow126) ? 1.0 : fSlow125));
	double fSlow127 = ((iSlow126) ? 0.0 : fThen49);
	...
	double fThen58 = std::exp(0.0 - fConst1 / ((iSlow137) ? 1.0 : fSlow136));
	double fSlow138 = ((iSlow137) ? 0.0 : fThen58);
	double fSlow139 = 1.0 - fSlow138;
	int iSlow140 = std::fabs(fSlow135) < 2.2204460492503131e-16;
	double fThen61 = std::exp(0.0 - fConst1 / ((iSlow140) ? 1.0 : fSlow135));
	double fSlow141 = ((iSlow140) ? 0.0 : fThen61);
	double fSlow142 = double(fHslider33);
	int iSlow143 = std::fabs(fSlow142) < 2.2204460492503131e-16;
	double fThen63 = std::exp(0.0 - fConst1 / ((iSlow143) ? 1.0 : fSlow142));
	double fSlow144 = ((iSlow143) ? 0.0 : fThen63);
	for (int i0 = 0; i0 < count; i0 = i0 + 1) {
		...
		double fTemp14 = ((fRec10[1] > fTemp13) ? fSlow56 : fSlow53);
		fRec11[0] = fTemp13 * (1.0 - fTemp14) + fTemp14 * fRec11[1];
		fRec10[0] = fRec11[0];
		fRec9[0] = fSlow46 * fSlow51 * std::max<double>(fSlow2 + 20.0 * std::log10(std::max<double>(2.2250738585072014e-308, fRec10[0])), 0.0) + fSlow50 * fRec9[1];
		...
		double fTemp33 = ((fRec28[1] > fTemp32) ? fSlow80 : fSlow77);
		fRec29[0] = fTemp32 * (1.0 - fTemp33) + fTemp33 * fRec29[1];
		fRec28[0] = fRec29[0];
		fRec27[0] = fSlow70 * fSlow75 * std::max<double>(fSlow59 + 20.0 * std::log10(std::max<double>(2.2250738585072014e-308, fRec28[0])), 0.0) + fSlow74 * fRec27[1];
		...
		double fTemp50 = ((fRec46[1] > fTemp49) ? fSlow104 : fSlow101);
		fRec47[0] = fTemp49 * (1.0 - fTemp50) + fTemp50 * fRec47[1];
		fRec46[0] = fRec47[0];
		fRec45[0] = fSlow94 * fSlow99 * std::max<double>(fSlow83 + 20.0 * std::log10(std::max<double>(2.2250738585072014e-308, fRec46[0])), 0.0) + fSlow98 * fRec45[1];
		...
		double fTemp65 = ((fRec62[1] > fTemp64) ? fSlow127 : fSlow124);
		fRec63[0] = fTemp64 * (1.0 - fTemp65) + fTemp65 * fRec63[1];
		fRec62[0] = fRec63[0];
		fRec61[0] = fSlow117 * fSlow122 * std::max<double>(fSlow107 + 20.0 * std::log10(std::max<double>(2.2250738585072014e-308, fRec62[0])), 0.0) + fSlow121 * fRec61[1];
		...
		double fTemp76 = ((fRec75[1] > fTemp75) ? fSlow144 : fSlow141);
		fRec76[0] = fTemp75 * (1.0 - fTemp76) + fTemp76 * fRec76[1];
		fRec75[0] = fRec76[0];
		fRec74[0] = fSlow134 * fSlow139 * std::max<double>(fSlow130 + 20.0 * std::log10(std::max<double>(2.2250738585072014e-308, fRec75[0])), 0.0) + fSlow138 * fRec74[1];
...
int Dsp::register_par(const ParamReg& reg)
{
	...
	reg.registerFloatVar("mbcs.Ratio1","","S",N_("Compression ratio"),&fHslider31, 2.0, 1.0, 100.0, 0.10000000000000001, 0);
	reg.registerFloatVar("mbcs.Ratio2","","S",N_("Compression ratio"),&fHslider25, 2.0, 1.0, 100.0, 0.10000000000000001, 0);
	reg.registerFloatVar("mbcs.Ratio3","","S",N_("Compression ratio"),&fHslider19, 2.0, 1.0, 100.0, 0.10000000000000001, 0);
	reg.registerFloatVar("mbcs.Ratio4","","S",N_("Compression ratio"),&fHslider13, 2.0, 1.0, 100.0, 0.10000000000000001, 0);
	reg.registerFloatVar("mbcs.Ratio5","","S",N_("Compression ratio"),&fHslider7, 2.0, 1.0, 100.0, 0.10000000000000001, 0);
	reg.registerFloatVar("mbcs.Attack1","","S",N_("Time before the compressor starts to kick in"),&fHslider32, 0.012, 0.001, 1.0, 0.001, 0);
	reg.registerFloatVar("mbcs.Attack2","","S",N_("Time before the compressor starts to kick in"),&fHslider26, 0.012, 0.001, 1.0, 0.001, 0);
	reg.registerFloatVar("mbcs.Attack3","","S",N_("Time before the compressor starts to kick in"),&fHslider20, 0.012, 0.001, 1.0, 0.001, 0);
	reg.registerFloatVar("mbcs.Attack4","","S",N_("Time before the compressor starts to kick in"),&fHslider14, 0.012, 0.001, 1.0, 0.001, 0);
	reg.registerFloatVar("mbcs.Attack5","","S",N_("Time before the compressor starts to kick in"),&fHslider8, 0.012, 0.001, 1.0, 0.001, 0);
