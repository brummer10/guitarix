...
Dsp::Dsp()
	...
	shortname = N_("MB Compresssor St");     // shortname
	mono_audio = 0;
...
void always_inline Dsp::compute(int count, float *input0, float *input1, float *output0, float *output1)
{
	...
	for (int i=0; i<count; i++) {
		...
		fRec12[0] = ((fSlow48 * max((fSlow47 + (20 * log10(fRec13[0]))), 0)) + (fSlow37 * fRec12[1]));
		...
		fRec20[0] = ((fSlow74 * max((fSlow73 + (20 * log10(fRec21[0]))), 0)) + (fSlow52 * fRec20[1]));
		...
		fRec32[0] = ((fSlow99 * max((fSlow98 + (20 * log10(fRec33[0]))), 0)) + (fSlow78 * fRec32[1]));
		...
		fRec44[0] = ((fSlow112 * max((fSlow111 + (20 * log10(fRec45[0]))), 0)) + (fSlow103 * fRec44[1]));
...
{
	reg.registerVar("mbcs.Attack1 (sec)","","S",N_("Time before the compressor starts to kick in"),&fslider28, 0.012, 0.0, 1.0, 0.001);
	static const value_pair fslider29_values[] = {{"Compress"},{"Bypass"},{"Mute"},{0}};
	reg.registerEnumVar("mbcs.Mode1","","S",N_("Compress or Mute the selected band, or Bypass The Compresssor"),fslider29_values,&fslider29, 1.0, 1.0, 3.0, 1.0);
	reg.registerVar("mbcs.Ratio2","","S",N_("Compresssion ratio"),&fslider26, 2.0, 1.0, 1e+02, 0.1);
	reg.registerVar("mbcs.Makeup-Threshold2 (db)","","S",N_("Threshold correction, an anticlip measure"),&fslider27, 2.0, 0.0, 1e+01, 0.1);
	reg.registerVar("mbcs.Release2 (sec)","","S",N_("Time before the compressor releases the sound"),&fslider24, 1.25, 0.0, 1e+01, 0.01);
	reg.registerVar("mbcs.Makeup2 (db)","","S",N_("Post amplification and threshold"),&fslider25, 1e+01, -5e+01, 5e+01, 0.1);
	reg.registerVar("mbcs.show_always.Crossover B1-B2 (hz)","","S",N_("Crossover bandpass frequency"),&fslider22, 8e+01, 2e+01, 2e+04, 1.0);
	static const value_pair fslider23_values[] = {{"Compress"},{"Bypass"},{"Mute"},{0}};
	reg.registerEnumVar("mbcs.Mode2","","S",N_("Compress or Mute the selected band, or Bypass The Compresssor"),fslider23_values,&fslider23, 1.0, 1.0, 3.0, 1.0);
	reg.registerVar("mbcs.Makeup-Threshold3 (db)","","S",N_("Threshold correction, an anticlip measure"),&fslider20, 2.0, 0.0, 1e+01, 0.1);
	reg.registerVar("mbcs.Attack2 (sec)","","S",N_("Time before the compressor starts to kick in"),&fslider21, 0.012, 0.0, 1.0, 0.001);
	reg.registerVar("mbcs.Makeup-Threshold5 (db)","","S",N_("Threshold correction, an anticlip measure"),&fslider13, 2.0, 0.0, 1e+01, 0.1);
	reg.registerVar("mbcs.Ratio5","","S",N_("Compresssion ratio"),&fslider12, 2.0, 1.0, 1e+02, 0.1);
	reg.registerVar("mbcs.Makeup5 (db)","","S",N_("Post amplification and threshold"),&fslider11, 11.0, -5e+01, 5e+01, 0.1);
	reg.registerVar("mbcs.Release5 (sec)","","S",N_("Time before the compressor releases the sound"),&fslider10, 1.25, 0.0, 1e+01, 0.01);
	reg.registerVar("mbcs.Release3 (sec)","","S",N_("Time before the compressor releases the sound"),&fslider17, 1.25, 0.0, 1e+01, 0.01);
	static const value_pair fslider16_values[] = {{"Compress"},{"Bypass"},{"Mute"},{0}};
	reg.registerEnumVar("mbcs.Mode3","","S",N_("Compress or Mute the selected band, or Bypass The Compresssor"),fslider16_values,&fslider16, 1.0, 1.0, 3.0, 1.0);
	reg.registerVar("mbcs.show_always.Crossover B2-B3 (hz)","","S",N_("Crossover bandpass frequency"),&fslider15, 2.1e+02, 2e+01, 2e+04, 1.0);
	reg.registerVar("mbcs.Attack3 (sec)","","S",N_("Time before the compressor starts to kick in"),&fslider14, 0.012, 0.0, 1.0, 0.001);
	reg.registerVar("mbcs.Ratio3","","S",N_("Compresssion ratio"),&fslider19, 2.0, 1.0, 1e+02, 0.1);
	reg.registerVar("mbcs.Makeup3 (db)","","S",N_("Post amplification and threshold"),&fslider18, 4.0, -5e+01, 5e+01, 0.1);
	reg.registerVar("mbcs.Makeup1 (db)","","S",N_("Post amplification and threshold"),&fslider31, 13.0, -5e+01, 5e+01, 0.1);
	reg.registerVar("mbcs.Release1 (sec)","","S",N_("Time before the compressor releases the sound"),&fslider30, 1.25, 0.0, 1e+01, 0.01);
	reg.registerVar("mbcs.Makeup-Threshold1 (db)","","S",N_("Threshold correction, an anticlip measure"),&fslider33, 2.0, 0.0, 1e+01, 0.1);
	reg.registerVar("mbcs.Ratio1","","S",N_("Compresssion ratio"),&fslider32, 2.0, 1.0, 1e+02, 0.1);
	reg.registerVar("mbcs.Makeup-Threshold4 (db)","","S",N_("Threshold correction, an anticlip measure"),&fslider7, 2.0, 0.0, 1e+01, 0.1);
	reg.registerVar("mbcs.Ratio4","","S",N_("Compresssion ratio"),&fslider6, 2.0, 1.0, 1e+02, 0.1);
	reg.registerVar("mbcs.Makeup4 (db)","","S",N_("Post amplification and threshold"),&fslider5, 8.0, -5e+01, 5e+01, 0.1);
	reg.registerVar("mbcs.Release4 (sec)","","S",N_("Time before the compressor releases the sound"),&fslider4, 1.25, 0.0, 1e+01, 0.01);
	static const value_pair fslider3_values[] = {{"Compress"},{"Bypass"},{"Mute"},{0}};
	reg.registerEnumVar("mbcs.Mode4","","S",N_("Compress or Mute the selected band, or Bypass The Compresssor"),fslider3_values,&fslider3, 1.0, 1.0, 3.0, 1.0);
	reg.registerVar("mbcs.show_always.Crossover B4-B5 (hz)","","S",N_("Crossover bandpass frequency"),&fslider2, 5e+03, 2e+01, 2e+04, 1.0);
	reg.registerVar("mbcs.show_always.Crossover B3-B4 (hz)","","S",N_("Crossover bandpass frequency"),&fslider1, 1.7e+03, 2e+01, 2e+04, 1.0);
	reg.registerVar("mbcs.Attack4 (sec)","","S",N_("Time before the compressor starts to kick in"),&fslider0, 0.012, 0.0, 1.0, 0.001);
	static const value_pair fslider9_values[] = {{"Compress"},{"Bypass"},{"Mute"},{0}};
	reg.registerEnumVar("mbcs.Mode5","","S",N_("Compress or Mute the selected band, or Bypass The Compresssor"),fslider9_values,&fslider9, 1.0, 1.0, 3.0, 1.0);
	reg.registerVar("mbcs.Attack5 (sec)","","S",N_("Time before the compressor starts to kick in"),&fslider8, 0.012, 0.0, 1.0, 0.001);
	return 0;
}
