...
namespace gx_engine
{
  ...
  dsp() {}
    ...
    virtual void buildUserInterface(gx_ui::GxUI* interface) = 0;
    virtual void compute(int len, float** inputs, float** outputs) = 0;
    virtual void compute_midi( int len) = 0;

    bool isInitialized() { return fSamplingFreq != 0; }

    static void turnOnMidi () { midistate = kMidiOn;  }
    static void turnOffMidi() { midistate = kMidiOff; }
    static bool isMidiOn   () { return (midistate == kMidiOn ? true : false);  }
  ...
  public:
    ...
    void buildUserInterface(gx_ui::GxUI* interface);
