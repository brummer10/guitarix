...
//----------- small knob
    if (regler->regler_type == 0)
    {
        ...
        if (GTK_WIDGET_HAS_FOCUS(widget)== TRUE)
        {
            ...
            gdk_draw_pixbuf(GDK_DRAWABLE(widget->window), widget->style->fg_gc[0], GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->regler_image1, 0,0, reglerx, reglery, GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->regler_x, GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->regler_y, GDK_RGB_DITHER_NORMAL, 0, 0);
...
//--------- Big knob
    else if (regler->regler_type == 1)
    {
        ...
        if (GTK_WIDGET_HAS_FOCUS(widget)== TRUE)
        {
            ...
            gdk_draw_pixbuf(GDK_DRAWABLE(widget->window), widget->style->fg_gc[0], GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->bigregler_image1, 0,0, reglerx, reglery, GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->bigknob_x, GTK_REGLER_CLASS(GTK_OBJECT_GET_CLASS(widget))->bigknob_y, GDK_RGB_DITHER_NORMAL, 0, 0);
...
//--------- small knob size and steps
    klass->regler_x = 25;
    klass->regler_y = 25;
    klass->regler_step = 93;
//--------- big knob size and steps
    klass->bigknob_x = 50;
    klass->bigknob_y = 50;
    klass->bigknob_step = 93;
...
//----------- Big knob
    klass->bigregler_image2 = gdk_pixbuf_new_from_xpm_data (knob_big_xpm);
    klass->bigregler_image = gdk_pixbuf_scale_simple(klass->bigregler_image2,4800,50,GDK_INTERP_NEAREST);
    gdk_pixbuf_copy_area(klass->bigregler_image2,0,0,2400,50, klass->bigregler_image,0,0);
    klass->bigregler_image1 = gdk_pixbuf_flip (klass->bigregler_image2,TRUE);
    gdk_pixbuf_copy_area(klass->bigregler_image1,0,0,2350,50, klass->bigregler_image,2400,0);
    g_object_unref(klass->bigregler_image2 );
...
//----------- small knob
    klass->regler_image = gdk_pixbuf_scale_simple(klass->bigregler_image,2400,25,GDK_INTERP_HYPER);
