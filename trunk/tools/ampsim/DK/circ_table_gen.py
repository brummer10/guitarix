...
class Circ_table(gen):
    ...
    V = ''
    model = ''
    def set_model(self,model, S, V, M=None, D=None, O=None):
        self.model = model
        # D_ = D2
        self.S = S
        self.V = V
        ...
        if (M) :
            self.max_sig = M
            self.operator = 1.0/self.max_sig * self.set_operator
        if (D) :
            self.divider = D
...
"""
    ...
    def generate_table(self, p, y, filename=None):
        ...
        aj = 0
        if not self.divider :
            max_s = copy.deepcopy(y[len(self.sig)-1])
            if max_s[0] > 1:
                self.divider = max_s[0] *1.2
               # print ("table divder = %s") % self.divider
            else :
                self.divider = 1.0
        t1 = ("\n // max_sig = %f") % self.max_sig
        t2 = ("\n // step operator = %f") % self.operator
        t3 = ("\n // divider = %f\n") % self.divider
        self.table_define = self.table_define.replace('table1d', 'table%s' % self.model)
        self.table_use = self.table_use.replace('circ', '%s' % self.model)
        sys.stdout = open(n, 'w')
        sys.stdout.write("\n")
    ...
    def __init__(self, name, S, V, M=None, D=None, O=None):
        self.set_model(name, S, V, M, D, O)
