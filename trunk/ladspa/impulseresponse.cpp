...
//
// Abstract definitirion of a User Interface to be passed to the 
// buildUserInterface methodir of a Faust Signal Processor
// 
//----------------------------------------------------------------
...
//
//  Abstract definitirion of a Faust Signal Processor
//
//----------------------------------------------------------------
...
class dsp {
 ...
 public:
	...
	virtual void initir(int samplingRate) 							= 0;
...
class irdsp : public dsp {
  ...
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "impulseresponse");
	...
	static void classinitir(int samplingFreq) {
	}
	virtual void instanceinitir(int samplingFreq) {
	...
	virtual void initir(int samplingFreq) {
		classinitir(samplingFreq);
		instanceinitir(samplingFreq);
	}
...
//-----------------------------------portCollectorir--------------------------------------
//
// portCollectorir is passed to the buildUserInterface methodir of a dsp object
...
class portCollectorir : public UI
{		
 private:
	...
	void addPortDescrir(int type, const char* label, int hint, float min=0.0, float max=0.0) 
	{
 ...
 public:
	...
	portCollectorir(int ins, int outs) : UI(), fInsCount(ins), fOutsCount(outs), fCtrlCount(0) 
	{
		for (int i = 0; i < ins; i++) { 
	...
	virtual void addNumDisplay(const char* label, float* zone, int precision) {
		addPortDescrir(OCONTROL, label, 0, -10000, +10000); 
	}
	virtual void addTextDisplay(const char* label, float* zone, char* names[], float min, float max) {
		addPortDescrir(OCONTROL, label, LADSPA_HINT_BOUNDED_BELOW | LADSPA_HINT_BOUNDED_ABOVE, min, max); 
	}
...
{	
 ...
 public:
	...
	void setPortDatair (unsigned long port, LADSPA_Data* data) {
		fPortData[port] = data;
	}
...
LADSPA_Descriptor* 	gDescriptori = 0;
...
LADSPA_Handle instantiate_methodir (const struct _LADSPA_Descriptor * Descriptor, unsigned long SampleRate) 
{
	dsp*		p = new irdsp();
...
void connect_methodir (LADSPA_Handle Instance, unsigned long Port, LADSPA_Data * DataLocation) 
{
	PLUGIN* p = (PLUGIN*) Instance;
	p->fPortData->setPortDatair(Port, DataLocation);
}

void activate_methodir (LADSPA_Handle Instance)
{
	PLUGIN* p = (PLUGIN*) Instance;
	p->fDsp->initir(p->fSampleRate);
}

void run_methodir (LADSPA_Handle Instance, unsigned long SampleCount)
{
	PLUGIN* p = (PLUGIN*) Instance;
...
void deactivate_methodir (LADSPA_Handle Instance)
{
}

void cleanup_methodir (LADSPA_Handle Instance)
{
	PLUGIN* p = (PLUGIN*) Instance;
	delete p->fPortData;
	delete p->fDsp;
...
void initir_descriptor(LADSPA_Descriptor* descriptor) 
{
	...
	// description des methodirs
	descriptor->instantiate = instantiate_methodir;
	descriptor->connect_port = connect_methodir;
	descriptor->activate = activate_methodir;
	descriptor->run = run_methodir;
	descriptor->run_adding = 0;
	descriptor->set_run_adding_gain = 0;
	descriptor->deactivate = deactivate_methodir;
	descriptor->cleanup = cleanup_methodir;
}
...
const LADSPA_Descriptor * ladspa_descriptor(unsigned long Index) 
{
    if (Index == 0) {
		if (gDescriptori == 0) 
		{
			// allocate temporaries dsp and portCollectorir to build the plugin description
			irdsp* p = new irdsp();
			if (p) {
				portCollectorir*	c=new portCollectorir(p->getNumInputs(), p->getNumOutputs());
				p->buildUserInterface(c);
				gDescriptori = new LADSPA_Descriptor;
				initir_descriptor(gDescriptori);
				c->fillPortDescription(gDescriptori);
				delete p;
			} else {
